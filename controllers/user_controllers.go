...
import (
	...
	"github.com/Johna210/go-jwt-project/models"
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if count > 0 {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "this email exists"})
			return
		}
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while chcking for the phone"})
			return
		}

		if count > 0 {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "this phone number exists"})
			return
		}
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		token, refreshToken, _ := helper.GenerateAllTokens(*foundUser.Email, *foundUser.First_name,
			*foundUser.Last_name, *foundUser.User_type, foundUser.User_id)
